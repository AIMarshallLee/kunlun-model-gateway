...
渠道运营台支持允许目录中的平台密钥配置／轮换、禁用和原操作查询；确认预览不含密钥，PUT 断线也按未知写入结果处理、不自动重试。权限、保密范围和验收见 [渠道运营台说明](docs/CHANNEL-CONSOLE-ACCEPTANCE.md)。凭据启用不代表供应商批准或健康检查通过。

商业购买入口现在要求至少一个上架模型具有已启用的允许渠道；停供时隐藏新购买并在服务端拒绝创建支付会话，已有订单与付款回调继续处理。详见 [购买与供给门禁](docs/PURCHASE-SUPPLY-ACCEPTANCE.md)。此检查不外呼探活，也不证明真实凭据或官方支付已验收。
