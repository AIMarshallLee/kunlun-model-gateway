...
export function createCheckoutFlow(request, origin, uuid = () => crypto.randomUUID()) {
  ...
  return {
    ...
    async start(selectedSku) {
      if (busy) throw new Error(t("已有购买请求正在处理，请等待或查询原订单。"));
      if (key && sku !== selectedSku) throw new Error(t("请先核对原购买请求，再明确开始新的购买。"));
    ...
    async lookup() {
      if (!key) throw new Error(t("当前页面没有购买请求编号，请从订单列表查询。"));
      return send("/billing/checkout/lookup", {method: "POST", headers: {"Idempotency-Key": key}});
    },
    forget() {
      if (busy) throw new Error(t("购买请求仍在处理中，暂不能开始另一笔购买。"));
      clear();
    },
    clear,
  };
