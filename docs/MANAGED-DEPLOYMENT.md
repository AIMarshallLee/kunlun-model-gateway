...
备份恢复新增[隔离 PostgreSQL 自动化验收](RESTORE-ACCEPTANCE.md)：新空库恢复后比较完整合成数据并复查账本、未知费用占用和权限。它不替代真实 Supabase 加密／项目恢复验收；旧 Compose 覆盖恢复入口已明确停用。

运行层新增[双进程 HTTP 负载与硬停止恢复验收](RUNTIME-ACCEPTANCE.md)：在 PostgreSQL 上验证有限并发、跨进程幂等、进程被 kill 后的占用保留与替代实例。它不代表 Vercel 实际重部署、跨版本升级／回滚或长时容量测试通过。

购买入口已新增[已配置供给一致性门禁](PURCHASE-SUPPLY-ACCEPTANCE.md)：没有允许渠道支持当前上架模型时，公共目录及控制台暂停新购买，checkout 接口在创建订单前再次拒绝。它只证明配置元数据状态，不验证实际 Key 健康或供应商授权；`/readyz` 仍是运行配置／Vault 读取检查，不能单独作为真实模型可调用或商业发布证据。
