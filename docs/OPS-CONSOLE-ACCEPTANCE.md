...
| `alerts:read/write` | 后续新增当前告警聚合及审计确认；确认不是解除告警，详见[运营告警验收](OPS-ALERTS-ACCEPTANCE.md) |
...
本批不是完整 P0-10：外部告警通知闭环、更多 Key 历史的检索体验、退款风险回收/核销的完整浏览器情景及真实 IdP/MFA/入口验收仍待完成。当前告警聚合与确认已有[后续验收](OPS-ALERTS-ACCEPTANCE.md)；模型价格上下架/版本操作界面已有[独立验收](MODEL-PRICE-ACCEPTANCE.md)；可视化渠道配置、轮换、禁用和原操作查询见[渠道后续验收](CHANNEL-CONSOLE-ACCEPTANCE.md)。客户资料与风险原因只可交付给获授权运营者。

后续已补齐超过 100 个 Key 的分页和精确 ID 检索，含旧 Key 冻结、只读权限、语言切换与退出竞态的本地浏览器验收，见[历史 Key 检索验收](OPS-KEY-HISTORY-ACCEPTANCE.md)。不据此更新本节其他未验收项。
